...
private:
    ...
    VectorXd Z_sd_; MatrixXd Z_std_;
    ...
    void class_setup(VectorXd L, VectorXd R, VectorXi Rinf, VectorXi P2, MatrixXd X, MatrixXd Z, MatrixXd W, double r, int ngrid){
        ...
        for (int i=0;i<n_;++i) { xiXexpXW_[i].resize(pX_); xiX2expXW_[i].resize(pX_,pX_);}
        for (int l=0;l<m_;++l) VilX_[l].resize(n_,pX_);
        cal_w();
        logLik_i_.setZero();
    ...
    void uniqueXZ(){
        Z_sd_.resize(pZ_); VectorXd Z_mean(pZ_); Z_sd_.setZero();
        for (int j=0;j<pZ_;++j) Z_mean(j) = Z_.col(j).mean();
        ...
        for (int i=0;i<n_;++i){
            ...
            for (int j=0;j<pZ_;++j) Z_sd_(j) += pow(Z_(i,j)-Z_mean(j),2);
        }
        nx_ = (long)linex; nz_ = (long)linez;
        x_ = x_.block(0,0,linex,pX_); z_ = z_.block(0,0,linez,pZ_);
        
        Z_std_.resize(n_,pZ_);
        for (int j=0;j<pZ_;++j) {
            Z_sd_(j) = sqrt(Z_sd_(j)/((double)n_-1.0)); Z_std_.col(j) = Z_.col(j) / Z_sd_(j);
        }
        cout<<"nx="<<nx_<<", nz= "<<nz_<<endl;
    }
    
    void cal_w(){
        an = pow((double)n_,-0.25); wiv_.setZero(); VectorXd wi(n_);
        for (int i=0;i<n_;++i){
            for (int j=0;j<n_;++j) wi(j) = K_epan((Z_std_.row(i)-Z_std_.row(j))/an);
            wi /= wi.sum();
            for (int j=0;j<n_;++j) wiv_(i,Zline_(j)) += wi(j);
        }
    }
    ...
    MatrixXd calint(VectorXd beta, VectorXd gamma, VectorXd lambda, MatrixXd puv) {
        ...
        for (int i=0;i<n_;++i){
            ...
            if (P2_(i)==1){
                expXW = exp(W_.row(i)*gamma) * exp(X_.row(i)*beta);
                for (int s=0;s<ngrid_;++s){
                    ...
                    if (Rinf_(i)==0) {
                        ...
                        for (int l=(Lline_(i)+1);l<=Rline_(i);++l){
                            ...
                            VilX_[l].row(i) += mus * lik_xi * xi * lambda(l) * expXW / denom * X_.row(i);
                        }
                    }
                    xiexpXW_(i) += mus * lik_xi * xi * expXW;
                    xiXexpXW_[i] += mus * lik_xi * xi * expXW * X_.row(i).transpose();
                    xiX2expXW_[i] += mus * lik_xi * xi * expXW * X_.row(i).transpose() * X_.row(i);
                }
                ...
                for (int u=0;u<nx_;++u){
                    expXW = exp(W_.row(i)*gamma) * exp(x_.row(u)*beta);
                    ...
                    for (int s=0;s<ngrid_;++s){
                        ...
                        xiXexpXW_[i] += mus * lik_xiu * xi * expXW * x_.row(u);
                        xiX2expXW_[i] += mus * lik_xiu * xi * expXW * x_.row(u).transpose() * x_.row(u);
                        if (Rinf_(i)==0) {
                            denom = 1-exp(-xi * expXW * (Lambda(Rline_(i))-Lambda(Lline_(i))));
                            for (int l=(Lline_(i)+1);l<=Rline_(i);++l){
                                Vil_(i,l) += mus * lik_xiu * xi * lambda(l) * expXW /denom;
                                VilX_[l].row(i) += mus * lik_xiu * xi * lambda(l) * expXW /denom * x_.row(u);
                            }
                        }
                    }
    ...
    VectorXd Mstep(VectorXd& beta, VectorXd& gamma){
        ...
        for (int l=(int)m_-1;l>=0;--l){
            for (int i=0;i<n_;++i){
                if (l==Rstar_(i)){
                    sumtheta += xiexpXW_(i);
                    sumthetaZ.head(pX_) += xiXexpXW_[i];
                    sumthetaZ.tail(pW_) += xiexpXW_(i) * W_.row(i).transpose();
                    sumthetaZZ.block(0,0,pX_,pX_) += xiX2expXW_[i];
                    sumthetaZZ.block(pX_,0,pW_,pX_) += W_.row(i).transpose() * xiXexpXW_[i].transpose();
                    sumthetaZZ.block(0,pX_,pX_,pW_) = sumthetaZZ.block(pX_,0,pW_,pX_).transpose();
                    sumthetaZZ.block(pX_,pX_,pW_,pW_) += xiexpXW_(i) * W_.row(i).transpose() * W_.row(i);
                }
            }
            lambda(l) = Vil_.col(l).sum()/sumtheta;
            for (int p=0;p<pX_;++p) Score_(p) += VilX_[l].col(p).sum();
        ...
        beta += step.head(pX_); gamma += step.tail(pW_);
        return(lambda);
    }
    ...
    VectorXd profilelogLik_i(VectorXd beta,VectorXd gamma){
        ...
        while (iter_<=maxiter_){
            ...
            iter_++;
            if ((diff(lambda,lambda0)<epsilon_lambda)) break;//&(abs(logLik_i_.mean()-lik0_)<epsilon_lik)
        }
        return(logLik_i_);
    }
    ...
    void Profile_I(double h){
        ...
        for (int j=0;j<pX_;++j){ beta=beta_; gamma = gamma_; beta(j)+=h; pL_i.col(j)=profilelogLik_i(beta,gamma);}
        for (int j=0;j<pW_;++j){ beta=beta_; gamma = gamma_; gamma(j)+=h; pL_i.col(pX_+j)=profilelogLik_i(beta,gamma);}
...
TwoPhase simudata(long nsub, VectorXd beta, VectorXd gamma, default_random_engine & generator, double r, int ngrid){
    ...
    for (int i=0;i<nsub;++i){
        ...
        Z2 = Udist(generator); Z3 = Udist(generator) * sin(X(i,0));
...
TwoPhase simudata_X(long nsub, VectorXd beta, VectorXd gamma, default_random_engine & generator, double r, int ngrid){
    //Distributions
    uniform_real_distribution<double> Xdist(-0.5,0.5);
    uniform_real_distribution<double> Udist(0,1);
    uniform_real_distribution<double> Cdist(0,3);
    ...
    for (int i=0;i<nsub;++i){
        ...
        T = genaT(0.1, r, expXW, generator); //Lambda(t) = 0.1t
        C = Cdist(generator);
        vector<double> CC=genC(0, C, 0, 0.4*exp(X(i,0)/2.0), generator, nint);
        long nC = CC.size();
        if (nC==0){ L(i)=0; R(i)=1; Rinf(i)=1; nR++; }
        else {
            if (T<CC[0]) {L(i)=0; R(i)=CC[0]; Rinf(i)=0;}
...
TwoPhase simudata_overZ(long nsub, VectorXd beta, VectorXd gamma, default_random_engine & generator, double r, int ngrid){
    //Distributions
    uniform_real_distribution<double> Xdist(-0.5,0.5);
    uniform_real_distribution<double> Udist(0,1);
    uniform_real_distribution<double> Cdist(0,3);
    bernoulli_distribution P2_dist(0.2);
    bernoulli_distribution P2_dist_tail(0.4);
    ...
    double T, C, expXW, Z2, Z3;
    MatrixXd Z0(nsub,1); VectorXi index0(nsub); int line = 0;
    for (int i=0;i<nsub;++i){
        X(i,0) = Xdist(generator);
        Z2 = Udist(generator); Z3 = Udist(generator) * sin(X(i,0));
        Z(i,0) = Z3; W.row(i) << Z3, Z2;
        ...
        if (Rinf(i)==1) { Z0.row(line) = Z.row(i); index0(line) = i; line++;}
    }
    VectorXd Zsort = VecSort(Z0.col(0).head(line));
    int index1 = (int) (((double)line - 1.0) * 0.25);
    int index3 = (int) (((double)line - 1.0) * 0.75);
    double q1 = Zsort(index1);
    double q3 = Zsort(index3);
    for (int i=0;i<line;++i){
        if ((Z0(i,0)<=q1)|(Z0(i,0)>q3)) P2(index0(i)) = P2_dist_tail(generator);
        else P2(index0(i)) = P2_dist(generator);
    }
    cout<<"left-censored: "<<nL/(double)nsub<<endl;
    cout<<"interval-censored: "<<nI/(double)nsub<<endl;
    cout<<"right-censored: "<<nR/(double)nsub<<endl;
...
TwoPhase simudata_depZ(long nsub, VectorXd beta, VectorXd gamma, default_random_engine & generator, double r, int ngrid){
    //Distributions
    uniform_real_distribution<double> Xdist(-0.5,0.5);
    uniform_real_distribution<double> Udist(0,1);
    uniform_real_distribution<double> Cdist(0,3);
    ...
    for (int i=0;i<nsub;++i){
        ...
        if (Rinf(i)==1) {
            double logitp = -0.9+Z(i,0);
            bernoulli_distribution P2_dist(exp(logitp)/(1+exp(logitp)));
            P2(i) = P2_dist(generator);
        }
    }
    cout<<"left-censored: "<<nL/(double)nsub<<endl;
    cout<<"interval-censored: "<<nI/(double)nsub<<endl;
...
TwoPhase simudata_depZ_mis(long nsub, VectorXd beta, VectorXd gamma, default_random_engine & generator, double r, int ngrid){
    //Distributions
    uniform_real_distribution<double> Xdist(-0.5,0.5);
    uniform_real_distribution<double> Udist(0,1);
    uniform_real_distribution<double> Cdist(0,3);
